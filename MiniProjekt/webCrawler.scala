...
// Sagt dem Scala-Compiler, dass er die cats-effect-Bibliothek in Version 3.6.1 verwenden soll
...
object WebCrawler extends IOApp {
  ...
  // Lädt den HTML-Inhalt einer gegebenen URL herunter (in den RAM als String)
  ...
  // Dies ist eine pure Funktion, da sie keine Nebeneffekte hat und bei gleichen Eingaben immer gleiche Ausgaben liefert
      ...
      for {
        ...
        result <- htmlOpt match {                  // Pattern Matching auf das Ergebnis des Downloads
          case Some(html) =>
            ...
            // Nimmt nur so viele Links, wie noch Seiten übrig sind (gemäss maxPages)
          ...
          case None =>
            // Falls der Download fehlschlägt. Protokolliert den Fehler und gibt einen leeren CrawlNode zurück
  ...
  def saveResult(result: CrawlNode, path: String): IO[Unit] = IO {
    ...
    writer.close() // Wichtig: Den Writer schliessen, um Ressourcen freizugeben
  }
